...
class NoCourseException{
    private:
        int index;
    public:
        NoCourseException(const int index){
            this->index = index;
        }
        const void print(){
            cout << "Demonstratorot so indeks " << this->index << " ne drzi laboratoriski vezbi\n"; 
        }
};
...
public:
    ...
    virtual const int getBodovi(){
        double count=0;
        for (int i=0; i<this->brojOcenki; i++){
            if(this->ocenki[i] > 5)
                count++;
        }
...
class Demonstrator : public Student, public Predavach{
    private:
        int noHours;
    public:
        Demonstrator(int indeks, int *ocenki, int brojOcenki, char *imeIPrezime, Kurs *kursevi, int brojKursevi, int noHours) : Student(indeks, ocenki, brojOcenki), Predavach(imeIPrezime, kursevi, brojKursevi){
            this->noHours = noHours;
        }
        const int getBodovi(){
            if(!this->getBrojKursevi())
                throw NoCourseException(this->indeks);
            return Student::getBodovi() + (20*this->noHours)/this->getBrojKursevi();
        }
        const void pecati(){
            cout << this->indeks << ": ";
            Predavach::pecati();
