...
class Transakcija{
    ...
    public:
        ...
        virtual const double voEvra() = 0;
        virtual const double voDolari() = 0;
...
class DeviznaTransakcija : public Transakcija {
    ...
    DeviznaTransakcija(){
        ...
        const double voEvra(){
            if (!(strcmp(this->currency, "EUR")))
                return this->total;
            else
                return (this->total * this->usd) / this->eur;  
        }
        const double voDolari(){
            if(!(strcmp(this->currency, "USD")))
                return this->total;
            else
                return (this->total * this->eur) / this->usd;
        }
        const void pecati(){
...
class Smetka{
    ...
    public:
        ...
        const void izvestajVoEvra(){
            double total = 0.0;
            for(int i=0; i<this->noTransactions; i++)
                total += this->tii[i]->voEvra();
            cout << "Korisnikot so smetka: " << this->smetka << " ima momentalno saldo od " << (this->saldo / Transakcija::getEUR()) + total << " EUR" << endl;
        }
        const void izvestajVoDolari(){
            double total = 0.0;
            for(int i=0; i<this->noTransactions; i++)
                total += this->tii[i]->voDolari();
            cout << "Korisnikot so smetka: " << this->smetka << " ima momentalno saldo od " << (this->saldo / Transakcija::getUSD()) + total << " USD" << endl;
        }
